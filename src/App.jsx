    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
        {ccys.map(c => {
          ...
          return <KPI key={c} label={`${c}/MYR`} value={f4(v)} color={ccyColor(c)} up={d > 0} down={d < 0} sub={d != null ? fpct(d) + " vs prior" : "—"} />;
        })}
      </div>
      ...
      <Card title="Entities" accent={P.mag}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 10 }}>
          {store.entities.map(e => (
            <div key={e.id} style={{ background: P.panel, borderRadius: 10, padding: "11px 13px", borderLeft: `3px solid ${e.active ? e.color : P.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: e.active ? P.text : P.muted, marginBottom: 3 }}>{e.name}</div>
              <div style={{ fontSize: 11, color: P.muted }}>{e.country} · <Badge label={e.currency} color={ccyColor(e.currency)} /> · {e.type}</div>
            </div>
          ))}
        </div>
      </Card>
...
function FXRates() {
  ...
  // Rebased data
  const rb = useMemo(() => rebase(fxRates, foreignCcys, baseCcy), [fxRates, foreignCcys, baseCcy]);
  const last = rb[rb.length - 1] || {};
  const prev = rb[rb.length - 2] || {};
  const periods = fxRates.map(r => r.period);

  // Weekly detection
  const isWeekly = useMemo(() => isWeeklyData(periods), [fxRates]);
  const effectiveMode = viewMode === "auto" ? (isWeekly ? "weekly" : "monthly") : viewMode;
  ...
  function startEdit() { setEditData(JSON.parse(JSON.stringify(fxRates))); setEditing(true); }
  ...
  function saveEdit() {
    ...
    const ns = { ...store, fxRates: cleaned };
  ...
  function addPeriod() {
    ...
    if (!p || fxRates.find(r => r.period === p)) return;
    const ns = { ...store, fxRates: [...fxRates, { period: p }] };
  ...
  function delPeriod(p) {
    ...
    const ns = { ...store, fxRates: fxRates.filter(r => r.period !== p) };
    setStore(ns); saveStore(ns);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={toolbar}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <select value={viewMode} onChange={e => setViewMode(e.target.value)} style={selStyle}>
            <option value="auto">Auto ({isWeekly ? "weekly" : "monthly"})</option>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 10 }}>
        {foreignCcys.map(c => {
          const v = last[c], pv = prev[c], d = pct(pv, v);
          return <KPI key={c} label={`${c}/${baseCcy}`} value={f4(v)} color={ccyColor(c)} up={d > 0} down={d < 0} sub={d != null ? fpct(d) + " vs prior" : "—"} />;
        })}
      </div>
      ...
      {/* RATE TABLE */}
      {tab === "table" && (
        <Card noPad title={`X per 1 ${baseCcy} · ${rb.length} ${effectiveMode === "weekly" ? "dates" : "periods"}`} accent={P.gold}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                <thead><tr style={{ background: P.panel }}>
                  <th style={{ padding: "9px 14px", textAlign: "left", color: P.muted, borderBottom: `1px solid ${P.border}`, fontSize: 10, fontWeight: 600, letterSpacing: 1, textTransform: "uppercase" }}>{effectiveMode === "weekly" ? "Date" : "Period"}</th>
...
function ImportData() {
  ...
  function parseFile(file) {
    ...
    reader.onload = e => {
      try {
        ...
        const pk = keys.find(k => /period|month|date/i.test(k));
        if (!pk) { setParseErr('No "Period", "Month" or "Date" column found.'); return; }
        const parsed = rows.map(r => {
          const o = { period: String(r[pk] || "").trim() };
          keys.forEach(k => { if (k === pk) return; const v = parseFloat(r[k]); if (!isNaN(v) && v >= 0) o[k.toUpperCase().trim()] = v; });
          return o;
        }).filter(r => r.period);
        if (!parsed.length) { setParseErr("No valid rows found."); return; }
  ...
  function confirmImport() {
    if (!preview) return;
    const entry = { id: uid(), filename: preview.filename, importedAt: new Date().toISOString(), periods: preview.count, importedBy: "admin", snapshot: store.fxRates };
    const ns = { ...store, fxRates: preview.rows, importHistory: [entry, ...history].slice(0, 20) };
  ...
  async function importGSheet() {
    ...
    try {
      ...
      const hdrs = rows[0].map(h => h.toUpperCase().trim());
      const pi = hdrs.findIndex(h => /PERIOD|MONTH|DATE/.test(h));
      if (pi < 0) throw new Error('No Period/Date column found.');
      const parsed = rows.slice(1).map(row => {
        const o = { period: row[pi] || "" };
        hdrs.forEach((h, i) => { if (i !== pi && row[i] && !isNaN(parseFloat(row[i]))) o[h] = parseFloat(row[i]); });
        return o;
      }).filter(r => r.period);
      if (!parsed.length) throw new Error("No valid rows.");
      const entry = { id: uid(), filename: "Google Sheets", importedAt: new Date().toISOString(), periods: parsed.length, importedBy: "admin", snapshot: store.fxRates };
      const ns = { ...store, fxRates: parsed, importHistory: [entry, ...history].slice(0, 20) };
      setStore(ns); saveStore(ns); setGsStatus("ok");
    } catch (e) { setGsStatus("error:" + e.message); }
  }

  function restore(entry) {
    if (!window.confirm(`Restore data from "${entry.filename}"? Current data will be replaced.`)) return;
    const ns = { ...store, fxRates: entry.snapshot };
    setStore(ns); saveStore(ns);
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <PageTitle title="Import Data" sub="Replace current FX rates from file or Google Sheets" />
      <Tabs tabs={[{ id: "excel", label: "📂 Excel Upload" }, { id: "gsheet", label: "📊 Google Sheets" }]} active={tab} onChange={setTab} />

      {tab === "excel" && (
        <Card title="Upload Excel File" accent={P.gold}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ fontSize: 12, color: P.muted, lineHeight: 1.7 }}>
              Upload <code style={{ color: P.gold, background: P.panel, padding: "1px 5px", borderRadius: 4 }}>.xlsx</code> or <code style={{ color: P.gold, background: P.panel, padding: "1px 5px", borderRadius: 4 }}>.xls</code> file.<br />
              Required columns: <code style={{ color: P.gold, background: P.panel, padding: "1px 5px", borderRadius: 4 }}>Period | MYR | USD | SGD | PHP</code>
            </div>
            <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); const f = e.dataTransfer.files[0]; if (f) parseFile(f); }}
              ...
              <input ref={fileRef} type="file" accept=".xlsx,.xls" onChange={e => { const f = e.target.files[0]; if (f) parseFile(f); e.target.value = ""; }} style={{ display: "none" }} />
            </div>
            ...
            {preview && (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ fontSize: 12, color: P.green }}>✓ Parsed {preview.count} periods from <strong>{preview.filename}</strong></div>
                    ...
                    <tbody>
                      {preview.rows.slice(0, 6).map((r, i) => (
                        <tr key={i} style={{ borderBottom: `1px solid ${P.border}30`, background: i % 2 ? P.panel + "60" : "transparent" }}>
                          {Object.keys(r).map(k => <td key={k} style={{ padding: "5px 12px", textAlign: k === "period" ? "left" : "right", fontFamily: k === "period" ? "inherit" : "'DM Mono',monospace", color: P.text, fontSize: 11 }}>{r[k]}</td>)}
                        </tr>
                      ))}
                      {preview.rows.length > 6 && <tr><td colSpan={99} style={{ padding: "5px 12px", color: P.muted, fontSize: 10 }}>…and {preview.rows.length - 6} more rows</td></tr>}
                    </tbody>
                  </table>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
        ...
        <Card title="Google Sheets Import" accent={P.teal}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontSize: 12, color: P.muted, lineHeight: 1.7 }}>
              ...
              Columns: <code style={{ color: P.gold, background: P.panel, padding: "1px 5px", borderRadius: 4 }}>Period | MYR | USD | SGD | PHP</code>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            {history.map(h => (
              <div key={h.id} style={{ background: P.panel, border: `1px solid ${P.border}`, borderRadius: 10, padding: "11px 13px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: P.muted }}>{fdate(h.importedAt)} · {h.periods} periods · {h.importedBy}</div>
                </div>
...
function Settings() {
  ...
  const [exFmt, setExFmt] = useState("excel");
  ...
  function exportPDF() {
    const fccys = (store.currencies || DEFAULT_CURRENCIES).filter(c => c !== BASE_DEFAULT);
    const rbRows = rb.map(r => `<tr>${["period", ...fccys].map(c => `<td style="padding:5px 10px;text-align:${c==="period"?"left":"right"};border-bottom:1px solid #eee;font-family:monospace">${c==="period" ? r.period : (r[c] != null ? Number(r[c]).toFixed(4) : "—")}</td>`).join("")}</tr>`).join("");
    ...
    const html = `<!DOCTYPE html><html><head><title>FXFlow Export</title><style>body{font-family:Arial,sans-serif;padding:28px;color:#111}h1{font-size:20px;margin-bottom:4px}h2{font-size:14px;margin:20px 0 8px;color:#555}table{width:100%;border-collapse:collapse;font-size:12px}th{padding:6px 10px;text-align:left;background:#f5f5f5;border-bottom:2px solid #ddd;font-size:10px;text-transform:uppercase;letter-spacing:1px}.meta{font-size:11px;color:#888;margin-bottom:20px}</style></head><body>
      <h1>FXFlow — FX Rate Report</h1>
      <div class="meta">Generated: ${new Date().toLocaleString("en-GB")} · Base: MYR</div>
      ${exRates ? `<h2>FX Rates (X per 1 MYR)</h2><table><thead><tr>${["Period", ...fccys].map(h => `<th>${h === "period" ? "Period" : `${h}/MYR`}</th>`).join("")}</tr></thead><tbody>${rbRows}</tbody></table>` : ""}
      ${exEntities ? `<h2>Entities</h2><table><thead><tr><th>ID</th><th>Name</th><th>Country</th><th>Currency</th><th>Type</th></tr></thead><tbody>${entRows}</tbody></table>` : ""}
    </body></html>`;
  ...
  const chk = (val, set, label) => (
    <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: P.text }}>
      <input type="checkbox" checked={val} onChange={e => set(e.target.checked)} style={{ width: 15, height: 15 }} />{label}
    </label>
  );

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <PageTitle title="Settings" sub={`FXFlow v2.0 · ${STORAGE_KEY}`} />
      <Card title="Export" accent={P.gold}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 10, color: P.muted, letterSpacing: 1, textTransform: "uppercase", marginBottom: 2 }}>Include</div>
              {chk(exRates, setExRates, "FX Rates (raw + rebased)")}
              {chk(exEntities, setExEntities, "Entities")}
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ fontSize: 10, color: P.muted, letterSpacing: 1, textTransform: "uppercase", marginBottom: 2 }}>Format</div>
              {["excel", "pdf"].map(f => (
                <label key={f} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: P.text }}>
                  <input type="radio" name="fmt" checked={exFmt === f} onChange={() => setExFmt(f)} />{f === "excel" ? "Excel (.xlsx)" : "PDF (print)"}
                </label>
              ))}
            </div>
          </div>
          <Btn onClick={exFmt === "excel" ? exportExcel : exportPDF} color={P.gold} disabled={!exRates && !exEntities}>
            {exFmt === "excel" ? "📥 Export Excel" : "🖨 Export PDF"}
          </Btn>
        </div>
      </Card>
      <Card title="Active Currencies" accent={P.mag}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {(store.currencies || DEFAULT_CURRENCIES).map(c => (
            <div key={c} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 10, background: P.panel, border: `1px solid ${ccyColor(c)}30` }}>
              <div style={{ width: 7, height: 7, borderRadius: "50%", background: ccyColor(c) }} />
              <span style={{ fontSize: 13, fontWeight: 600, color: ccyColor(c) }}>{c}</span>
              {CCY_SYM[c] && <span style={{ fontSize: 11, color: P.muted }}>{CCY_SYM[c]}</span>}
              {c === BASE_DEFAULT && <Badge label="Base" color={P.mag} />}
            </div>
          ))}
        </div>
        <div style={{ marginTop: 8, fontSize: 11, color: P.muted }}>Add currencies in FX Rates → + Add.</div>
      </Card>
      <Card title="Data" accent={P.border}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div><div style={{ fontSize: 13, fontWeight: 600, color: P.text, marginBottom: 3 }}>Reset to Defaults</div><div style={{ fontSize: 12, color: P.muted }}>All data and import history will be lost.</div></div>
          <Btn onClick={reset} color={P.red} outline>Reset</Btn>
        </div>
      </Card>
    </div>
  );
